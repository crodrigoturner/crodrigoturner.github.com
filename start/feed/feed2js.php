...
if (isset($restrict_url) && substr($src_host, strlen($src_host)-strlen($restrict_url)) != $restrict_url) {
	...
	if  (!$rss) {
		...
		if ($chan == 'y') {
			...
			$str.= "document.write('<p class=\"rss-title\"><a target=\"_blank\" class=\"rss-title\" href=\"" . trim($rss->channel['link']) . '"' . $target_window . ">" . addslashes(strip_returns($rss->channel['title'])) . "</a><span class=\"rss-item\">" . addslashes(strip_returns(strip_tags($rss->channel['description']))) . "</span></p>');\n";
		
		} elseif ($chan == 'title') {
			// output title only with link
			$str.= "document.write('<p class=\"rss-title\"><a target=\"_blank\" class=\"rss-title\" href=\"" . trim($rss->channel['link']) . '"' . $target_window . ">" . addslashes(strip_returns($rss->channel['title'])) . "</a></p>');\n";
		...
		foreach ( $all_items as $item ) {
			...
			if ($desc < 0) {
				...
				$str.= "document.write('<li class=\"rss-item\"><a target=\"_blank\" class=\"rss-item\" href=\"" . trim($my_url) . "\"" . $target_window . ' >' . $my_title . '</a>' .  $author_str . "');\n";
