...
$.fn.lifestream.feeds.flickr = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<span class="webapp flickr">Flickr</span> He subido una foto <a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.foursquare = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      checkedin: '<span class="webapp foursquare">Foursquare</span> He estado en  @ <a href="${link}">${title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.github = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      commitCommentEvent: 'Github: He comentado en <a href="http://github.com/' +
        '${status.repo.name}">${status.repo.name}</a>',
      createBranchEvent: 'Github: He creado un branch en <a href="http://github.com/' +
        '${status.repo.name}/tree/${status.payload.ref}">' +
        '${status.payload.ref}</a> at <a href="http://github.com/' +
        '${status.repo.name}">${status.repo.name}</a>',
      createRepositoryEvent: 'Github: He creado un repository en ' +
      ...
      createTagEvent: 'created tag <a href="http://github.com/' +
        ...
        '${status.payload.ref}</a> at <a href="http://github.com/' +
        '${status.repo.name}">${status.repo.name}</a>',
      deleteBranchEvent: 'Github: He eliminado un branch en ${status.payload.ref} at ' +
        '<a href="http://github.com/${status.repo.name}">' +
        '${status.repo.name}</a>',
      deleteTagEvent: 'deleted tag ${status.payload.ref} at ' +
        '<a href="http://github.com/${status.repo.name}">' +
        '${status.repo.name}</a>',
      followEvent: 'Github: He empezado a seguir a <a href="http://github.com/' +
      ...
      issueCommentEvent: 'Github: He comentado en un issue <a href="http://github.com/' +
      ...
      pullRequestEvent: '${status.payload.action} pull request ' +
        ...
        '<a href="http://github.com/${status.repo.name}">' +
        '${status.repo.name}</a>',
      pushEvent:'<span class="webapp github">Github</span> He actualizado <a href="http://github.com/${status.repo.name}' +
        '/tree/${status.payload.ref}">${status.payload.ref}</a> en ' +
        '<a href="http://github.com/${status.repo.name}">' +
        '${status.repo.name}</a>',
      watchEvent: 'Github: He empezado a seguir a <a href="http://github.com/' +
        '${status.repo.name}">${status.repo.name}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.googleplus = function( config, callback ) {
  ...
  var template = $.extend({},
    {
    posted: 'Google +: <a href="${actor.url}">${actor.displayName}</a>' +
      ' ha publicado una nueva entrada <a href="${url}" ' +
      'title="${id}">${title}</a> <!--With--> ' +
      '${object.replies.totalItems} respuestas, ' +
      '${object.plusoners.totalItems} +1s, ' +
      '${object.resharers.totalItems} veces compartido'
    },
    config.template),
...
$.fn.lifestream.feeds.hypem = function( config, callback ) {
  ...
  var template = $.extend({},
  {
    loved: 'Hypem: Me ha gustado <a href="http://hypem.com/item/${mediaid}">${title}</a> by <a href="http://hypem.com/artist/${artist}">${artist}</a>',
    history: 'Hypem: He escuchado <a href="http://hypem.com/item/${mediaid}">${title}</a> by <a href="http://hypem.com/artist/${artist}">${artist}</a>'
  },
  config.template);
...
$.fn.lifestream.feeds.pinboard = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      bookmarked: '<a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.pocket = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      pocketed: '<span class="webapp">Pocket</span> have saved <a href="${link}">${title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.posterous = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<i class="fab fa-get-pocket"></i> <a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.quora = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<span class="webapp quora">Quora</span> <a href="${link}">${title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.rss = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<span class="webapp"><i class="fa fa-wordpress" aria-hidden="true"></i></span> He publicado: <a href="${link}">${title}</a>'
    },
    config.template),
...
$.fn.lifestream.feeds.tumblr = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<span class="webapp tumblr">Tumblr</span> ${type} <a href="${url}">${title}</a>'
    },
    config.template),
  ...
  createTumblrOutput = function( config, post ) {
    return {
      ...
      html: $.tmpl( template.posted, {
          type: post.type.replace('photo', 'una foto'),
		  url: post.url,
          image: getImage(post),
          title: getTitle(post)
        } )
  ...
  $.fn.lifestream.feeds.twitter = function(config, callback) {
    var template = $.extend({},
      {
        "posted": '<i class="fab fa-twitter"></i> {{html tweet}}'
      },
      config.template);
...
$.fn.lifestream.feeds.vimeo = function( config, callback ) {
  ...
  var template = $.extend({},
  {
    liked: '<span class="webapp vimeo"><i class="fa fa-vimeo" aria-hidden="true"></i></span> <a href="${url}" title="${description}">${title}</a>',
    posted: '<span class="webapp vimeo"><i class="fa fa-vimeo" aria-hidden="true"></i></span> <a href="${url}" title="${description}">${title}</a>'
  },
  config.template),
...
$.fn.lifestream.feeds.wikipedia = function( config, callback ) {
  ...
  template = $.extend({},
    {
      contribution: '<span class="webapp"><i class="fa fa-wikipedia-w" aria-hidden="true"></i></span> He contribuido en <a href="${url}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.wordpress = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      posted: '<span class="wp">Wordpress</span> He publicado <a href="${link}">${title}</a>'
    },
    config.template);
...
$.fn.lifestream.feeds.youtube = function( config, callback ) {
  ...
  var template = $.extend({},
    {
      uploaded: '<span class="webapp youtube">Youtube</span> <a href="${video.player.default}" ' +
        'title="${video.description}">${video.title}</a>',
      favorited: '<span class="webapp youtube">Youtube</span>  <a href="${video.player.default}" ' +
        'title="${video.description}">${video.title}</a>'
    },
    config.template),
